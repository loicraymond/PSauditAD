...
# 
$usersNotConnectedSince = Get-ADUser -SearchBase "OU=Administratif,OU=UtilisateurSemitag,DC=semitag,DC=lan" -Filter {Enabled -eq $True -and LastLogonTimeStamp -lt $DelaiConnexion} -Properties Name, LastLogonTimestamp, displayName, physicalDeliveryOfficeName

If($usersNotConnectedSince -ne $NULL)
{
    ...
    Foreach($Utilisateur in $usersNotConnectedSince)
    {
...
# 
$usersWithNoChangePwd = Get-ADUser -Filter {Enabled -eq $True -and passwordNeverExpires -eq "True"} -Properties *

If($usersWithNoChangePwd -ne $NULL)
{
    ...
    foreach($user in $usersWithNoChangePwd)
    {  
        $MsgCorps += ("<tr><td>"+$user.cn+"</td><td>"+$user.sAMAccountName+"</td></tr>")
    }
    $MsgCorps += "</tbody></table>"
...
# 
$Utilisateurs = Get-ADUser -Filter {Enabled -eq $True -and AccountExpirationDate -ne "<Never>"} -Properties Name, accountExpires, displayName
...
Foreach($Utilisateur in $Utilisateurs)
{
    ...
    {
        $MsgCorps += ("<tr><td>"+$UtilisateurNom+"</td><td>"+$UtilisateurLogin+"</td><td>"+$DateExpiration+"</td></tr>")
    }
}
$MsgCorps += "</tbody></table>"
